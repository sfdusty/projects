...
def fetch_draftables(draftgroup_id):
    """Fetch draftable players for a specific draft group, only returning players with a salary."""
    url = DRAFTABLES_ENDPOINT.format(draftgroup_id=draftgroup_id)
    try:
        response = session.get(url, timeout=5)
        response.raise_for_status()
        data = response.json()
        # Only return players with a salary, using a dictionary to remove duplicates
        players = {
            player['displayName']: {
